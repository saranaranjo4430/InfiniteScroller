...
{	
    ...
    {
        return CSimpleSound::GetInstance().LoadSound(fileName);
    }
    bool PlaySound(const char *fileName, bool looping)
	{
		DWORD flags = (looping) ? DSBPLAY_LOOPING : 0;
		return CSimpleSound::GetInstance().PlaySound(fileName, flags);
	}
    ...
    {
        CSimpleSound::GetInstance().ReleaseSound(fileName);
    }
